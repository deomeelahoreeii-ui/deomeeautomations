...
## WhatsApp inbound file archive and exports

Bundle 1 stores inbound WhatsApp message and attachment metadata durably. Bundle 2
adds on-demand media retrieval and audited export packages. Open
<http://localhost:4321/whatsapp/inbound-files>, select an existing directory contact,
choose a captured date range, direct-chat or group scope, and one or more supported
file classes (images, PDFs, or spreadsheets), then scan before building the package.

The worker downloads media from the locally persisted Baileys message payload and
streams the bytes directly to the protected FastAPI endpoint. Binary content is never
sent through NATS. The platform enforces a size limit, verifies SHA-256, detects the
actual file type from content, archives the immutable media, and reuses it in later
exports. The generated ZIP includes category folders plus `manifest.csv`,
`manifest.json`, `unavailable-files.csv`, and `export-summary.json`.
...
POST /api/v1/whatsapp/inbound/attachments/{attachment_id}/content  (worker only)
```

The default media limit is 75 MiB per attachment. Configure both sides consistently:
...
`./scripts/dev.sh` applies Alembic migrations and starts one local Celery worker for
`antidengue,crm,whatsapp`. A production deployment should run the `whatsapp` queue in
a separate worker so large exports cannot delay unrelated jobs.

Historical coverage is intentionally reported as partial unless the message was
captured by the inbound worker. Expired or unavailable WhatsApp media is listed in the
audit files rather than silently omitted.
