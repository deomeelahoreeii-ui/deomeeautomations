...
class PreviewBatchError(RuntimeError):
    """Raised when a signed preview batch cannot be safely executed."""


class PreviewBatchChangedError(PreviewBatchError):
    def __init__(self, changes: list[dict[str, Any]]) -> None:
        self.changes = changes
        super().__init__(
            "The approved Helpdesk preview is no longer current; run preview again before syncing"
        )
...
class ComplaintHelpdeskSyncService:
    ...
    @staticmethod
    def _bounded_batch_limit(limit: int) -> int:
        return max(1, min(int(limit), 200))

    def _candidate_cases(self, *, limit: int) -> list[ComplaintCase]:
        """Return the exact deterministic candidate order used by preview and execution."""
        priority = sql_case(
            (ComplaintCase.frappe_ticket_id.is_(None), 0),
            (ComplaintCase.frappe_sync_status.in_(["failed", "update_pending"]), 1),
            else_=2,
        )
        return list(
            self.session.exec(
                ...
                .order_by(
                    priority.asc(),
                    col(ComplaintCase.created_at).asc(),
                    col(ComplaintCase.id).asc(),
                )
                .limit(self._bounded_batch_limit(limit))
            ).all()
        )

    def _case_preview_revision(self, case: ComplaintCase) -> str:
        _, _, payload_hash = ticket_payloads(
            case,
            self.settings,
            sync_version=case.frappe_sync_version + 1,
        )
        snapshot = {
            ...
            "frappe_payload_hash": case.frappe_payload_hash,
    ...
    def _preview_signing_key(self) -> bytes:
        secret = str(self.settings.frappe_helpdesk_api_secret or "").strip()
        if not secret:
            raise PreviewBatchError(
                "FRAPPE_HELPDESK_API_SECRET is required to sign an executable preview"
            )
        material = f"deomee-helpdesk-preview-v1:{secret}".encode("utf-8")
    ...
    def _encode_preview_token(self, payload: dict[str, Any]) -> str:
        ...
        signature = hmac.new(self._preview_signing_key(), body, hashlib.sha256).digest()
    ...
    def _decode_preview_token(self, token: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise PreviewBatchError("Helpdesk preview token payload is invalid") from exc
        if payload.get("version") != 1:
            raise PreviewBatchError("Unsupported Helpdesk preview token version")
        if payload.get("site") != self.settings.frappe_helpdesk_site:
            raise PreviewBatchError("Helpdesk preview token belongs to another Frappe site")
        if int(payload.get("expires_at") or 0) < int(time.time()):
            raise PreviewBatchError("Helpdesk preview token has expired; run preview again")
        if not isinstance(payload.get("cases"), list):
            raise PreviewBatchError("Helpdesk preview token has no case selection")
        return payload

    def preview(self, *, limit: int = 200) -> dict[str, Any]:
        cases = self._candidate_cases(limit=limit)
        items: list[dict[str, Any]] = []
        token_cases: list[dict[str, str]] = []
        ...
        for case in cases:
            ...
            revision = self._case_preview_revision(case)
            token_cases.append({"id": str(case.id), "revision": revision})
            items.append(
                {
                    "case_id": str(case.id),
        ...
        ttl = int(self.settings.frappe_helpdesk_preview_token_ttl_seconds)
        selection_digest = hashlib.sha256(
            json.dumps(token_cases, sort_keys=True, separators=(",", ":")).encode("utf-8")
        ).hexdigest()
        token_payload = {
            "version": 1,
            "site": self.settings.frappe_helpdesk_site,
        ...
        batch_token = None if configuration_errors else self._encode_preview_token(token_payload)
        return {
            "items": items,
            "counts": dict(counts),
            "selected_count": len(items),
            "selected_case_ids": [item["case_id"] for item in items],
            "batch_id": selection_digest[:16],
    ...
    def _validated_preview_case_ids(self, preview_token: str) -> tuple[list[uuid.UUID], dict[str, Any]]:
        payload = self._decode_preview_token(preview_token)
        case_ids: list[uuid.UUID] = []
        changes: list[dict[str, Any]] = []
        for expected in payload["cases"]:
            try:
                case_id = uuid.UUID(str(expected.get("id")))
            except (TypeError, ValueError, AttributeError):
                changes.append({"case_id": expected.get("id"), "reason": "invalid_case_id"})
                continue
            case = self.session.get(ComplaintCase, case_id)
            if case is None:
                changes.append({"case_id": str(case_id), "reason": "case_missing"})
                continue
            actual_revision = self._case_preview_revision(case)
                ...
                changes.append(
                    {
                        "case_id": str(case_id),
                        "complaint_number": case.complaint_number,
                        "reason": "case_changed_since_preview",
                    }
                )
                continue
            case_ids.append(case_id)
        if changes:
            raise PreviewBatchChangedError(changes)
    ...
    def sync_many(
        ...
        *,
        ...
        if preview_token:
            ids, token_payload = self._validated_preview_case_ids(preview_token)
        elif case_ids is not None:
            ids = list(dict.fromkeys(case_ids))[: self._bounded_batch_limit(limit)]
        else:
            raise PreviewBatchError(
                "Batch synchronization requires a signed preview token or explicit case IDs"
            )
        ...
        response: dict[str, Any] = {
            ...
            "requested": len(ids),
            "executed_case_ids": [str(case_id) for case_id in ids],
        }
        if token_payload is not None:
            response.update(
                {
                    "batch_id": str(token_payload.get("selection_digest") or "")[:16],
                    "preview_issued_at": token_payload.get("issued_at"),
                    "preview_expires_at": token_payload.get("expires_at"),
                }
            )
        return response

    def audit_links(self, *, limit: int = 200) -> dict[str, Any]:
        rows = list(
            self.session.exec(
                select(ComplaintCase)
                .where(ComplaintCase.frappe_ticket_id.is_not(None))
                .order_by(
                    col(ComplaintCase.frappe_last_synced_at).desc(),
                    col(ComplaintCase.created_at).asc(),
                    col(ComplaintCase.id).asc(),
                )
            ...
            "items": [
                {
                    ...
                    "frappe_ticket_url": case.frappe_ticket_url,
                    "frappe_sync_status": case.frappe_sync_status,
                    "frappe_sync_version": case.frappe_sync_version,
                    "frappe_last_synced_at": case.frappe_last_synced_at,
                }
                for case in rows
            ],
        }
