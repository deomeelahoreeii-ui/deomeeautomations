...
def render_consolidated_action_digest(
    ...
    *,
    ...
    for item in [
        ...
        *timing_report.issues,
        ...
        if item.get("code") == "nothing_to_dispatch":
            continue
        inherited = _parent_issue(item)
        if inherited is not None:
            issues.append(inherited)
    source_hashes = {
    ...
    presentation_metadata, evidence_issues = validate_digest_evidence(
        message=message,
        context=context,
        scope_key=scope_key,
        school_total=len(schools),
        workbook_available=path.is_file() and bool(digest),
        summarized_activity_count=int(
            timing_report.presentation_metadata.get("summarized_activity_count") or 0
        ),
    )
