...
class CrmBulkOperationService:
    ...
    def create_letter_batch(
        ...
        *,
        ...
        """Generate official letters only from immutable Approved/Issued revisions.

        Individual and batch generation share OfficialLetterService, the configured
        legal-size ODT template, signature profile, numbering controls and artifact
        checksums. Imported drafts are never eligible.
        """
        ...
        if scope == "ready":
            ...
            for case, revision in rows:
                ...
                if generated is None:
                    ready_rows.append((case, revision))
            rows = ready_rows
        if not rows:
            raise BulkOperationValidationError(
                "No approved or issued reply revisions match the formal-letter scope"
            )

        now = utcnow()
        official = OfficialLetterService(self.session, self.settings)
        ...
        batch = self._create_batch(
            ...
            settings={
                "document_formats": ["odt", "pdf"],
                "template_id": defaults["default_template_id"],
                "signature_profile_id": defaults["default_signature_id"],
        ...
        with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_DEFLATED) as archive:
            for case, revision in rows:
                ...
                item = CrmBulkOperationItem(
                    ...
                    reply_version_after=None,
                    reply_content_hash=revision.content_hash,
                    details_json={
                        "reply_revision_id": str(revision.id),
                ...
                try:
                    ...
                    artifacts = {entry["kind"]: entry for entry in result.get("artifacts", [])}
                    odt_record, odt_path = official.artifact_path(uuid.UUID(artifacts["odt"]["id"]))
                    pdf_record, pdf_path = official.artifact_path(uuid.UUID(artifacts["pdf"]["id"]))
                    odt_relative = f"{number}/{number} - DEO Official Letter.odt"
                    pdf_relative = f"{number}/{number} - DEO Official Letter.pdf"
                    archive.writestr(odt_relative, odt_path.read_bytes())
                    archive.writestr(pdf_relative, pdf_path.read_bytes())
                    item.status = "generated"
                    item.output_filename = pdf_relative
                    item.output_sha256 = pdf_record.sha256
                    item.processed_at = now
                    item.details_json = {
                        **item.details_json,
                        "official_letter_id": result["id"],
                        "letter_number": result["letter_number"],
                        "letter_date": result["letter_date"],
                        "odt_sha256": odt_record.sha256,
                        "pdf_sha256": pdf_record.sha256,
                    }
                    current_reply = self.session.exec(
                        select(ComplaintReply).where(ComplaintReply.complaint_case_id == case.id)
                    ).first()
                    if current_reply:
                    ...
                    item.status = "failed"
                    item.error_code = "official_letter_generation_failed"
            ...
            archive.writestr(
                "README.txt",
                "Official-letter batch generated only from immutable Approved/Issued reply revisions. "
                "Each complaint folder contains the configured legal-size ODT and PDF.\n",
            )
        if success:
            self._write_artifact(
                batch,
