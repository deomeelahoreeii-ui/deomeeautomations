...
class ComplaintTaxonomyService:
    ...
    def sync_category(self, category_id: uuid.UUID, *, previous_name: str | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            failed_category = self.session.get(ComplaintCategory, category_key)
            if failed_category is None:
                raise TaxonomyError(error) from exc
            failed_category.frappe_sync_status = "failed"
            failed_category.frappe_last_error = error
            failed_category.updated_at = utcnow()
            self.session.add(failed_category)
            self.session.commit()
            return {
                "status": "failed",
                "category_id": str(failed_category.id),
                "error": error,
            }
