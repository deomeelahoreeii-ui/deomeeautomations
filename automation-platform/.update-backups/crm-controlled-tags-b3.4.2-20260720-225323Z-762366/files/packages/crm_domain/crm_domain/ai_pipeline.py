...
class CrmAiPipelineService:
        ...
        *,
        ...
        tags = list(
            self.session.exec(
                ...
                .where(CrmComplaintTag.active == True)  # noqa: E712
        ...
        tag_rows = [[row.name, row.group_name, row.description or ""] for row in tags]
        example = _csv_bytes(
            CLASSIFICATION_HEADERS,
        ...
        with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_DEFLATED) as archive:
            ...
            archive.writestr("03-existing-tags.csv", _csv_bytes(["Tag", "Group", "Description"], tag_rows))
    ...
    def _tag_lookup(self) -> dict[str, CrmComplaintTag]:
        return {
            ...
            for row in self.session.exec(
                select(CrmComplaintTag).where(CrmComplaintTag.active == True)  # noqa: E712
            ).all()
        }
                ...
                if proposed_name:
                    self.session.add(
                        CrmTaxonomySuggestion(
                            ...
                            normalized_name=normalized_name(proposed_name).replace(" ", "-") if proposal_type == "tag" else normalized_name(proposed_name),
    ...
    def taxonomy_options(self) -> dict[str, Any]:
        ...
        return {
            ...
            "tags": [
                {"id": str(row.id), "name": row.name, "group": row.group_name, "description": row.description or ""}
                for row in tags
            ],
        }
