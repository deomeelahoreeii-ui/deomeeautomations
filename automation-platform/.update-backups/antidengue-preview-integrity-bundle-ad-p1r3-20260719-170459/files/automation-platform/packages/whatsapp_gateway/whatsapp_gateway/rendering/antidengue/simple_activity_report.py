...
def render_simple_activity_report(session: Session, *, source_job: Job, wing: Wing, recipient_name: str, scope_key: str, scope_value: str, scope_label: str, presentation_policy: dict[str, Any] | None = None, scope_values: list[str] | None = None) -> RenderedSimpleActivityReport:
    ...
    evidence_attachment_available = has_attachment or bool(
        policy.get("evidence_attachment_provided_by_parent")
    )
    details, omitted_schools, omitted_activities = _details(rows)
    if omitted_schools or omitted_activities:
        issues.append(_issue(
            "message_evidence_summarized"
            if evidence_attachment_available
            else "message_only_evidence_truncated",
            "info" if evidence_attachment_available else "blocked",
            (
                "WhatsApp text shows a concise evidence sample; the delivery workbook contains all scoped timing evidence."
                if evidence_attachment_available
                else
                "Message-only delivery cannot safely omit the remaining timing evidence. Enable an Excel attachment or narrow the rule."
            ),
            category="presentation",
            omitted_school_count=omitted_schools,
            omitted_activity_count=omitted_activities,
            evidence_attachment_source=(
                "component" if has_attachment else "parent"
                if evidence_attachment_available else "none"
            ),
        ))
    context = {
