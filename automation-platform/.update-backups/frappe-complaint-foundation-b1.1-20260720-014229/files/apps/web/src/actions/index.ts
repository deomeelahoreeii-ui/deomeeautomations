...
export const server = {
  ...
  crm: {
    ...
    syncHelpdeskBatch: defineAction({
      input: z.object({ caseIds: z.array(z.string().uuid()).max(200).default([]), limit: z.number().int().min(1).max(200).default(100), force: z.boolean().default(false) }),
      handler: ({ caseIds, limit, force }) => api("/api/v1/crm/helpdesk/sync", { method: "POST", body: JSON.stringify({ case_ids: caseIds, limit, force }) }),
    }),
    sheets: defineAction({
      input: z.object({
        search: z.string().default(""),
