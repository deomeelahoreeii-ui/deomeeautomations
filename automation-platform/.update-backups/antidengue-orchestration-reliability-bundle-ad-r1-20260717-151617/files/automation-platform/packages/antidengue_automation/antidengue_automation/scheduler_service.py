...
from antidengue_automation.scheduling import advance_pending_executions, ensure_due_executions
...
def run_tick() -> tuple[int, int]:
    # One scheduler process is started by dev.sh.  The PostgreSQL advisory lock
    # also prevents a second deployment instance from claiming the same state
    # machine concurrently; occurrence uniqueness remains the final safeguard.
...
def main() -> int:
    ...
    print(
        f"AntiDengue scheduler started: interval={interval:g}s, timezone=Asia/Karachi",
        flush=True,
    )
    while not _STOP:
        try:
            created, advanced = run_tick()
            if created or advanced:
                print(
                    f"AntiDengue scheduler tick: created={created}, advanced={advanced}",
                    flush=True,
                )
        except Exception as exc:  # The daemon must survive a transient DB/queue failure.
