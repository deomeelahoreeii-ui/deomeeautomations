...
def create_antidengue_run(
    ...
    if not request.dry_run:
        raise HTTPException(
            ...
            detail=(
                "Direct AntiDengue live mode is retired. Use Build and send now so the proven "
                "dry-run planner creates an immutable preview before any WhatsApp dispatch."
            ),
        )
    active_job = get_active_job(session, JobType.antidengue_report.value)
    if active_job is not None:
...
def create_antidengue_schedule(
    ...
    try:
        ...
        validate_dispatch_profile(session, data.dispatch_profile_id)
    ...
    item = AntiDengueSchedule(
        ...
        dispatch_profile_id=data.dispatch_profile_id,
...
def update_antidengue_schedule(
    ...
    if "dispatch_profile_id" in values:
        try:
            validate_dispatch_profile(session, values["dispatch_profile_id"])
        except ValueError as exc:
            raise HTTPException(status_code=422, detail=str(exc)) from exc
