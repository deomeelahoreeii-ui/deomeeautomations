...
def test_schedule_api_and_manual_execution_always_queue_dry_run(monkeypatch) -> None:
    ...
    monkeypatch.setattr("antidengue_automation.scheduling.run_antidengue_job.apply_async", lambda *args, **kwargs: FakeTask())
    try:
        with TestClient(app) as client:
            ...
            assert "Build and send now" in legacy_live.json()["detail"]
    finally:
