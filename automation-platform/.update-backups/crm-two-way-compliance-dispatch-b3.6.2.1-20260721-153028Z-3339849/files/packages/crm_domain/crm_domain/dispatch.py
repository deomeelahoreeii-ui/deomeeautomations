...
class CrmDispatchService:
    ...
    def _case_context(self, case: ComplaintCase) -> dict[str, Any]:
    ...
    def test_routing(self, case_id: uuid.UUID) -> dict[str, Any]:
    ...
    def create_batch(
        ...
        *,
        ...
        actor: str,
        purpose: str = "complaint_forwarding",
        response_due_at: datetime | None = None,
    ) -> dict[str, Any]:
        if not official_letter_ids:
            raise CrmDispatchValidationError("Select at least one finalized official letter")
        now = utcnow()
        ...
        batch = CrmDispatchBatch(
            ...
            total_items=len(official_letter_ids),
            purpose=purpose,
        ...
        for letter_id in official_letter_ids:
            if letter_id in seen:
                continue
            seen.add(letter_id)
            letter = self.session.get(CrmOfficialLetter, letter_id)
            if letter is None or letter.status != "finalized":
                raise CrmDispatchValidationError("Only finalized official letters can enter dispatch")
            ...
            if artifact is None or not Path(artifact.path).is_file():
                raise CrmDispatchValidationError(
                    f"Build the complete PDF for complaint {letter.complaint_number_snapshot} before dispatch"
                )
    ...
    def resolve_batch(self, batch_id: uuid.UUID) -> dict[str, Any]:
        ...
        for item in items:
            ...
            if case is None:
                ...
                self.session.add(item)
                continue
            matches = [rule for rule in enabled_rules if self._rule_matches(rule, self._case_context(case))]
            resolution = self._resolve_rule_set(matches)
    ...
    def set_manual_route(
        ...
        *,
        ...
        valid = {item["id"] for item in self.profiles(include_inactive=False)}
        if not profile_ids or any(str(value) not in valid for value in profile_ids):
    ...
    def compile_previews(self, batch_id: uuid.UUID, *, actor: str) -> dict[str, Any]:
        ...
        for profile_id, profile_targets in by_profile.items():
            ...
            for target in profile_targets:
                ...
                artifact = self.session.get(CrmOfficialLetterArtifact, item.complete_pdf_artifact_id)
                letter = self.session.get(CrmOfficialLetter, item.official_letter_id)
                case = self.session.get(ComplaintCase, item.complaint_case_id)
                if artifact is None or letter is None or case is None:
                    target.business_status = "blocked"
                    target.error = "Dispatch source record is missing"
                    self.session.add(target)
                    continue
                policy = dict(profile.presentation_policy or {})
                ...
                preview_artifact = WhatsAppDispatchPreviewArtifact(
                    ...
                    report_type_id=defaults["report_type"].id,
                    role="delivery",
                    name=f"{item.complaint_number_snapshot} - Complete Complaint Packet.pdf",
                ...
                values = {
                    ...
                    "instructions": "Please process in accordance with rules and policy.",
                }
    ...
    def statistics(self) -> dict[str, int]:
        ...
        ready_letters = int(self.session.scalar(
            select(func.count()).select_from(CrmOfficialLetter)
            .where(CrmOfficialLetter.status == "finalized")
        ) or 0)
        return {
            "ready_to_dispatch": ready_letters,
    ...
    def list_batches(self, *, search: str = "", status: str = "", page: int = 1, page_size: int = 25) -> dict[str, Any]:
        filters: list[Any] = []
        if status:
    ...
    def detail(self, batch_id: uuid.UUID) -> dict[str, Any]:
        ...
        for item in items:
            ...
            item_payload.append({
                ...
                "packet_download_url": f"/api/v1/crm/official-letters/artifacts/{item.complete_pdf_artifact_id}/download",
                "targets": [
                    {
                        "id": str(target.id),
