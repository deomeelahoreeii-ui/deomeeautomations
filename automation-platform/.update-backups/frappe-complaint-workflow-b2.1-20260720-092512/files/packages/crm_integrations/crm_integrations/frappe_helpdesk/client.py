    ...
    def open_assignments(self, ticket_name: str) -> list[dict[str, Any]]:
        return self.list_resources(
            ...
            filters=[
                ...
                ["ToDo", "reference_name", "=", ticket_name],
                ["ToDo", "status", "=", "Open"],
            ],
            fields=["name", "allocated_to", "assigned_by", "status", "description"],
            limit=100,
            order_by="creation asc",
        )

    def assign_to_ticket(
        self,
        ticket_name: str,
        users: Sequence[str],
        *,
        ...
        normalized = sorted({str(user).strip().lower() for user in users if str(user).strip()})
        if not normalized:
            return None
        args = {
            "doctype": "HD Ticket",
