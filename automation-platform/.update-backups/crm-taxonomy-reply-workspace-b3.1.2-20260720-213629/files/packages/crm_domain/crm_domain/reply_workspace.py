...
class ComplaintReplyWorkspaceService:
    ...
    def sync_classification(
        ...
        *,
        ...
        try:
            ...
            case.classification_sync_status = "failed"
            case.classification_last_error = str(exc)
            case.updated_at = utcnow()
            self._frappe_event(
                case,
                operation="sync_classification",
            ...
            return {
                ...
                "synchronized": False,
                "error": str(exc),
                "classification": self._classification_snapshot(case),
            }
    ...
    def bulk_classify(
        ...
        *,
        ...
        return {
            "results": results,
            "counts": dict(Counter(str(item.get("status") or "saved") for item in results)),
        ...
        *,
        ...
        try:
            ...
            if isinstance(exc, ReplyWorkspaceError):
                raise ReplyRemoteError(str(exc)) from exc
            raise ReplyRemoteError(str(exc)) from exc
