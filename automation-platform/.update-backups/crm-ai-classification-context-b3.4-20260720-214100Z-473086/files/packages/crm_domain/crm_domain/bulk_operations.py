    ...
    def validate_import_batch(
        ...
        *,
        ...
        if parent_batch_id:
            ...
            if parent.operation_type != "reply_export" or parent.status != "completed":
                raise BulkOperationValidationError("The related export batch must be completed")
            expected = {
            ...
            for item in valid_items:
                ...
                self.session.add(
                    ComplaintAuditEvent(
                        ...
                        after_json={"version": item.reply_version_after, "batch_id": str(batch.id)},
                        details_json={"batch_number": batch.batch_number, "source_row": item.source_row},
                    )
                )
