...
def test_export_import_and_letters_preserve_batch_lineage(tmp_path: Path) -> None:
    ...
    with Session(db) as session:
        ...
        approved = ComplaintReplyRevision(
            complaint_case_id=cases[0].id,
            reply_text=reply.reply_text,
            content_hash=hashlib.sha256(reply.reply_text.encode("utf-8")).hexdigest(),
            approval_status="Approved",
            source_reference="test-approved-revision",
        )
        ...
        with zipfile.ZipFile(letter_path) as archive:
            assert "104-8100001/104-8100001 - DEO Official Letter.odt" in archive.namelist()
            assert "104-8100001/104-8100001 - DEO Official Letter.pdf" in archive.namelist()
            assert "104-8100002/104-8100002 - DEO Official Letter.odt" not in archive.namelist()
