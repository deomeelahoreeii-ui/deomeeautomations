...
MASTER_DATA_CSS = ROOT / "apps/web/src/styles/master-data.css"
...
def test_officer_and_jurisdiction_pages_manage_primary_and_additional_charges() -> None:
    ...
    assert 'class="modal-box master-modal officer-assignment-modal"' in officers
    assert 'id="officer-additional-search"' in officers
    assert 'id="officer-additional-list"' in officers
    assert 'data-additional-ref=' in officers
    assert 'aria-pressed=' in officers
    assert "additionalSelection = new Set<string>()" in officers
    assert "jurisdiction_refs: jurisdictionRefs" in officers
    assert 'id="officer-replace-conflicts"' in officers
    assert 'id="officer-transfer-panel" hidden' in officers
    assert "Save officer & assignments" in officers
    assert 'id="officer-additional" multiple' not in officers
...
def test_officer_assignment_picker_has_complete_responsive_styling() -> None:
    styles = MASTER_DATA_CSS.read_text(encoding="utf-8")

    assert ".officer-assignment-modal" in styles
    assert ".officer-assignment-modal .master-form" in styles
    assert "overflow-y: auto" in styles
    assert ".jurisdiction-picker-list" in styles
    assert "grid-template-columns: repeat(2, minmax(0, 1fr))" in styles
    assert ".jurisdiction-option.selected" in styles
    assert ".jurisdiction-option.occupied:not(.selected)" in styles
    assert ".jurisdiction-transfer-warning" in styles
    assert '@media (max-width: 760px)' in styles
    assert '[data-theme="automation-dark"] .jurisdiction-picker' in styles
