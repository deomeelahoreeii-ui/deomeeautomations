...
def test_signed_preview_executes_exact_case_ids_even_if_candidate_order_changes() -> None:
    ...
    with Session(db) as session:
        base = utcnow()
        first = complaint(complaint_number="104-0000001", created_at=base)
        second = complaint(complaint_number="104-0000002", created_at=base + timedelta(seconds=1))
        third = complaint(complaint_number="104-0000003", created_at=base + timedelta(seconds=2))
        session.add_all([third, second, first])
        session.commit()

        service = ComplaintHelpdeskSyncService(session, settings(), client)
        preview = service.preview(limit=2)
        preview_ids = preview["selected_case_ids"]
        assert preview_ids == [str(first.id), str(second.id)]
        assert preview["batch_token"]

        # A new older complaint would have changed a fresh LIMIT query. The signed
        # batch must still execute only the exact IDs the operator approved.
        inserted_after_preview = complaint(
            complaint_number="104-0000000",
            created_at=base - timedelta(days=1),
        )
        session.add(inserted_after_preview)
        session.commit()

        result = service.sync_many(preview_token=preview["batch_token"])
        assert result["executed_case_ids"] == preview_ids
        assert [item["case_id"] for item in result["results"]] == preview_ids
        assert client.created_tickets == 2
        assert session.get(ComplaintCase, inserted_after_preview.id).frappe_ticket_id is None
        assert session.get(ComplaintCase, third.id).frappe_ticket_id is None


def test_signed_preview_aborts_whole_batch_when_case_changes() -> None:
    from crm_integrations.frappe_helpdesk.service import PreviewBatchChangedError

    db = engine()
    client = FakeHelpdeskClient()
    with Session(db) as session:
        first = complaint(complaint_number="104-0000010")
        second = complaint(complaint_number="104-0000011")
        session.add_all([first, second])
        session.commit()

        service = ComplaintHelpdeskSyncService(session, settings(), client)
        preview = service.preview(limit=2)
        second.remarks = "The complaint was edited after preview approval."
        session.add(second)
        session.commit()

        with pytest.raises(PreviewBatchChangedError) as exc_info:
            service.sync_many(preview_token=preview["batch_token"])
        assert client.created_tickets == 0
        assert exc_info.value.changes == [
            {
                "case_id": str(second.id),
                "complaint_number": second.complaint_number,
                "reason": "case_changed_since_preview",
            }
        ]


def test_preview_token_rejects_tampering_and_unsafe_implicit_selection() -> None:
    from crm_integrations.frappe_helpdesk.service import PreviewBatchError
    ...
    with Session(db) as session:
        ...
        token = preview["batch_token"]
        replacement = "A" if token[-1] != "A" else "B"
        tampered = token[:-1] + replacement

        with pytest.raises(PreviewBatchError, match="signature"):
            service.sync_many(preview_token=tampered)
        with pytest.raises(PreviewBatchError, match="requires a signed preview"):
            service.sync_many()
        assert client.created_tickets == 0


def test_explicit_case_ids_remain_supported_for_targeted_ui_actions() -> None:
    db = engine()
    client = FakeHelpdeskClient()
    with Session(db) as session:
        first = complaint(complaint_number="104-0000020")
        second = complaint(complaint_number="104-0000021")
        session.add_all([first, second])
        session.commit()
        result = ComplaintHelpdeskSyncService(session, settings(), client).sync_many(
            case_ids=[second.id]
        )
        assert result["executed_case_ids"] == [str(second.id)]
        assert session.get(ComplaintCase, first.id).frappe_ticket_id is None
        assert session.get(ComplaintCase, second.id).frappe_ticket_id is not None


def test_helpdesk_link_audit_maps_crm_cases_to_remote_tickets() -> None:
    db = engine()
    client = FakeHelpdeskClient()
    with Session(db) as session:
        case = complaint()
        ...
        session.commit()
        service = ComplaintHelpdeskSyncService(session, settings(), client)
        service.sync_case(case.id)
        audit = service.audit_links()
        assert audit["count"] == 1
        assert audit["items"][0]["case_id"] == str(case.id)
        assert audit["items"][0]["complaint_number"] == case.complaint_number
        assert audit["items"][0]["frappe_ticket_id"] == "HD-TICKET-00001"
