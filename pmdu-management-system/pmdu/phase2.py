...
class Phase2Settings:
    ...
    attachment_verify_ssl: bool
    pdf_enabled: bool
    pdf_format: str
    json_indent: bool
    max_files: int | None
    input_scope: str = "scraped-queue"
...
async def download_attachments(
    ...
    connector = aiohttp.TCPConnector(
        limit=settings.attachment_concurrency,
        ssl=settings.attachment_verify_ssl,
    )
    if not settings.attachment_verify_ssl:
        logging.getLogger(LOGGER_NAME).warning(
            "PMDU attachment SSL verification is disabled. This works around the CDN's incomplete certificate chain."
        )
...
def snapshot_attachments_ready(snapshot_path: Path, project_root: Path) -> bool:
    try:
    ...
    for attachment in attachments:
        ...
        if attachment.get("error"):
            return False
        if not attachment.get("cdn_url"):
            continue
        local_path = attachment.get("local_path")
        if not local_path:
            return False
        if not resolve_stored_path(str(local_path), project_root).exists():
            return False
    return True


def artifact_paths_exist(
    snapshot_path: Path,
    pdf_path: Path,
    pdf_required: bool,
    project_root: Path,
    ...
    if pdf_required and not pdf_path.exists():
        return False
    return snapshot_attachments_ready(snapshot_path, project_root)
...
def unchanged_source_is_ready(
    ...
    return artifact_paths_exist(snapshot_path, pdf_path, settings.pdf_enabled, project_root)
...
def queued_scraped_html_files(
    ...
    try:
        ...
        latest_run_id = ""
        if "last_seen_run_id" in queue_columns:
    ...
    if latest_run_id:
        logging.getLogger(LOGGER_NAME).info(
            "Phase Two latest scrape run id: %s", latest_run_id
        )

    files: list[Path] = []
    seen: set[Path] = set()
    for complaint_code, saved_path in rows:
        path = resolve_stored_path(str(saved_path), project_root)
        if not path.exists():
            logging.getLogger(LOGGER_NAME).warning(
                "Scrape queue raw HTML missing for %s: %s",
                complaint_code,
                saved_path,
            )
            continue
...
def phase2_input_files(
    conn: duckdb.DuckDBPyConnection,
    settings: Phase2Settings,
    project_root: Path,
) -> list[Path]:
    if settings.input_scope == "scraped-queue":
        return queued_scraped_html_files(conn, project_root)
    if settings.input_scope == "all-raw-html":
        return sorted(settings.raw_html_dir.glob("*.html"))
    raise ValueError(
        "PMDU_PHASE2_INPUT_SCOPE must be one of: scraped-queue, all-raw-html"
    )
...
def process_one_html(
    ...
    if duplicate:
        ...
        if artifact_paths_exist(snapshot_path, pdf_path, settings.pdf_enabled, project_root):
            logger.info(
...
def run_phase2(settings: Phase2Settings, project_root: Path) -> None:
    logger = logging.getLogger(LOGGER_NAME)
    raw_html_count = len(list(settings.raw_html_dir.glob("*.html")))
    ...
    logger.info("Phase Two input scope: %s", settings.input_scope)
    logger.info("Phase Two raw HTML files currently present: %s", raw_html_count)
    ...
    try:
        ...
        html_files = phase2_input_files(conn, settings, project_root)
        if settings.max_files is not None:
            html_files = html_files[: settings.max_files]
        logger.info("Phase Two HTML files selected for this run: %s", len(html_files))
        generated = 0
        rebuilt = 0
        skipped = 0
        unchanged = 0
