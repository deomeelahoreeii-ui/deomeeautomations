...
def load_settings(project_root: Path, require_credentials: bool = True) -> Settings:
    ...
    return Settings(
        ...
        diagnostics_dir=(
            project_root / os.getenv("PMDU_DIAGNOSTICS_DIR", "diagnostics/pmdu")
        ).resolve(),
        run_id=(
            os.getenv("PMDU_RUN_ID", "").strip()
...
def load_phase2_settings(project_root: Path):
    from pmdu.phase2 import Phase2Settings
    ...
    return Phase2Settings(
        ...
        attachment_verify_ssl=env_bool("PMDU_ATTACHMENT_VERIFY_SSL", False),
        ...
        input_scope=os.getenv("PMDU_PHASE2_INPUT_SCOPE", "scraped-queue").strip()
        or "scraped-queue",
    )
...
def load_paperless_settings(
    ...
    return PaperlessSettings(
        ...
        artifact_scope=(
            artifact_scope or os.getenv("PAPERLESS_ARTIFACT_SCOPE", "current-scrape")
        ).strip()
        or "current-scrape",
    )
...
def parse_paperless_flags(args: list[str]) -> dict[str, str | None]:
    ...
    while index < len(args):
        ...
        elif arg == "--artifact-scope":
            if index + 1 >= len(args):
                raise ValueError("--artifact-scope requires a value")
            artifact_scope = args[index + 1]
            index += 1
        elif arg.startswith("--artifact-scope="):
            artifact_scope = arg.split("=", 1)[1]
        else:
            raise ValueError(
                "Unknown Paperless flag: "
                + arg
                + ". Use --artifact-dir or --artifact-scope."
            )
        index += 1
    return {"artifact_dir": artifact_dir, "artifact_scope": artifact_scope}
...
def capture_artifacts(page: Page, settings: Settings, name: str) -> None:
    ...
    screenshot_path = capture_dir / f"{timestamp}-{name}.png"
    html_path = capture_dir / f"{timestamp}-{name}.html"
    metadata_path = capture_dir / f"{timestamp}-{name}.meta.json"
...
def build_scrape_queue(
    ...
    while True:
        ...
        inserted, requeued, existing = upsert_queue_rows(conn, new_rows, settings.run_id)
...
def main() -> int:
    ...
    try:
        ...
        if command in {"phase1", "scrape"}:
            ...
            from pmdu.phase2 import run_phase2

            project_root = Path(__file__).resolve().parent
            run_phase2(load_phase2_settings(project_root), project_root)
        elif command in {"phase3", "paperless", "paperless-upload"}:
