...
class PaperlessSettings:
    ...
    artifact_scope: str = "current-scrape"
...
def normalize_stored_path(value: str | None, project_root: Path) -> str:
    ...
    for marker in (
        ...
        "/deomeeautomations/pmdu-management-system/",
    ):
        if marker in value:
...
def latest_scrape_complaint_codes(
    ...
    try:
        ...
        if "last_seen_run_id" in columns:
            ...
            latest_run_id = str(latest_run[0]) if latest_run and latest_run[0] else ""
            if latest_run_id:
                rows = conn.execute(
                    """
...
def latest_snapshots(
    artifact_dir: Path,
    max_cases: int | None,
    allowed_complaint_codes: set[str] | None = None,
) -> list[Path]:
    ...
    for complaint_dir in sorted(
        ...
        if allowed_complaint_codes is not None and complaint_dir.name not in allowed_complaint_codes:
            continue
        versions = []
...
def snapshots_for_paperless_scope(
    settings: PaperlessSettings,
    conn: duckdb.DuckDBPyConnection,
) -> list[Path]:
    logger = logging.getLogger(LOGGER_NAME)
    scope = settings.artifact_scope.strip().lower()
    if scope == "current-scrape":
        allowed_complaint_codes = latest_scrape_complaint_codes(conn)
        logger.info(
            "Paperless artifact scope: current-scrape (%d complaint(s))",
            len(allowed_complaint_codes),
        )
    elif scope == "all-artifacts":
        allowed_complaint_codes = None
        logger.info("Paperless artifact scope: all-artifacts")
    else:
        raise ValueError(
            "PAPERLESS_ARTIFACT_SCOPE must be one of: current-scrape, all-artifacts"
        )
    return latest_snapshots(
        settings.artifact_dir,
        settings.max_cases,
        allowed_complaint_codes,
    )
...
def mark_paperless_document_stale(
    conn: duckdb.DuckDBPyConnection,
    paperless_document_id: int,
    error: str,
) -> None:
    conn.execute(
        ...
        SET upload_status = 'stale_remote_missing',
            uploaded_at = ?,
            error = ?
...
class PaperlessClient:
    def __init__(self, settings: PaperlessSettings) -> None:
        ...
        self.token = ""
        self._documents_by_title: dict[str, int] | None = None
    ...
    async def upload_document(
        ...
        document_id = await self.wait_for_document_id(task_id)
        self._documents_by_title = None
        return document_id

    async def find_document_by_title(self, title: str) -> int | None:
        if self._documents_by_title is None:
            documents = await self.paginated_results("/api/documents/?page_size=100")
            self._documents_by_title = {
                str(document.get("title", "")).strip().lower(): int(document["id"])
                for document in documents
                if document.get("id") and str(document.get("title", "")).strip()
            }
        return self._documents_by_title.get(title.strip().lower())

    async def get_document(self, document_id: int) -> dict[str, Any]:
        return await self.get_json(f"/api/documents/{document_id}/")

    async def get_document_or_none(self, document_id: int) -> dict[str, Any] | None:
        try:
...
def custom_field_payload(
    ...
    if data_type == "select":
        ...
        if isinstance(value, list):
            value = sorted({int(item) for item in value if item})
        else:
...
def apply_field_mapping(
    field_config: dict[str, Any], field_name: str, value: Any
) -> Any:
    if value in (None, ""):
        return value

    mappings = field_config.get("field_mappings", {})
    if not isinstance(mappings, dict):
        return value

    field_mapping = mappings.get(field_name)
    if field_mapping is None:
        field_mapping = mappings.get(field_name.lower())
    if not isinstance(field_mapping, dict):
        return value

    value_text = str(value).strip()
    for source, target in field_mapping.items():
        if str(source).strip().lower() == value_text.lower():
            return target
    return value
...
def build_custom_fields(
    ...
    for field_name, raw_value in resolved_role_field_config(
        ...
        value = apply_field_mapping(field_config, field_name, value)
...
def cleared_custom_field_value(field: dict[str, Any]) -> Any:
    if field.get("data_type") == "documentlink":
        return []
    return None


def apply_clear_custom_fields(
    custom_fields: list[dict[str, Any]],
    fields_by_name: dict[str, dict[str, Any]],
    ...
    for field_name in clear_field_names:
        ...
        if not field:
            continue
        updated = upsert_custom_field(
            updated,
            int(field["id"]),
            cleared_custom_field_value(field),
        )
    return updated


def documentlink_field_id(
    fields_by_name: dict[str, dict[str, Any]], field_name: str
) -> int:
    field = fields_by_name.get(field_name.lower())
    if not field:
        raise RuntimeError(f"Paperless custom field missing: {field_name}")
    if field.get("data_type") != "documentlink":
        raise RuntimeError(
            f"Paperless custom field {field_name} must be documentlink, "
            f"got {field.get('data_type')!r}"
        )
    return int(field["id"])
...
async def upload_artifact(
    ...
    try:
        ...
        if document_id:
            existing_document = await client.get_document_or_none(document_id)
            if existing_document is None:
                logger.warning(
                    "Tracked Paperless document is missing remotely for %s "
                    "(stale doc_id=%s). Will resolve by title or upload again.",
                    title,
                    document_id,
                )
                mark_paperless_document_stale(
                    conn,
                    document_id,
                    "Paperless API returned 404 for tracked document id.",
                )
                document_id = None
                existing_id = None
                baseline_fields = []
        ...
        if not document_id:
            ...
            if document_id:
                ...
                if existing_document is None:
                    logger.warning(
                        "Paperless title lookup returned stale doc_id=%s for %s. "
                        "Uploading a fresh document.",
                        document_id,
                        title,
                    )
                    document_id = None
                ...
                if document_id:
                    ...
                    if existing_document is None:
                        logger.warning(
                            "CRM complaint lookup returned stale doc_id=%s for %s. "
                            "Uploading a fresh document.",
                            document_id,
                            title,
                        )
        ...
        if sync_mode == "upload" and document_id:
            if not has_status_override:
                if artifact_role != "attachment":
                    ...
                    return document_id
                logger.info(
                    "Upload found existing attachment for %s (doc_id=%s); refreshing parent link.",
                    title,
                    document_id,
                )
            else:
        ...
        target_custom_fields = apply_clear_custom_fields(
            target_custom_fields,
            metadata["custom_fields"],
            clear_fields_for_role(field_config, artifact_role),
        )
...
async def find_existing_crm_main_complaint(
    ...
    for document in documents:
        ...
        number = custom_field_label(custom_fields, fields_by_name, "Complaint Number")
        if number.strip() != complaint_code:
            continue
    ...
    newest = sorted(matches, key=sort_key, reverse=True)[0]
...
async def patch_main_complaint_attachments(
    client: PaperlessClient,
    settings: PaperlessSettings,
    metadata: dict[str, Any],
    ...
    logger = logging.getLogger(LOGGER_NAME)
    field_id = documentlink_field_id(
        metadata["custom_fields"], "Complaint Attachments"
    )
    parent_case_field_id = documentlink_field_id(
        metadata["custom_fields"], "Parent Case"
    )
    attachment_ids = sorted({int(item) for item in attachment_document_ids if item})

    if settings.dry_run:
        logger.info(
            "DRY RUN: would set Complaint Attachments on doc_id=%s to %s.",
            parent_document_id,
            attachment_ids,
        )
        return

    document = await client.get_document(parent_document_id)
    live_fields = document.get("custom_fields", [])
    target_fields = upsert_custom_field(live_fields, parent_case_field_id, [])
    target_fields = upsert_custom_field(target_fields, field_id, attachment_ids)
    if target_fields == live_fields:
        logger.info(
            "Complaint Attachments already current for main doc_id=%s.",
            parent_document_id,
        )
        return
    ...
    if document.get("correspondent"):
        patch_payload["correspondent"] = document.get("correspondent")

    await client.patch_json(f"/api/documents/{parent_document_id}/", patch_payload)
    logger.info(
        "Set Complaint Attachments on main doc_id=%s to %s attachment document(s).",
        parent_document_id,
        len(attachment_ids),
    )
...
async def sync_one_snapshot(
    ...
    for attachment in snapshot.get("attachments", []):
        ...
        if unsupported_reason:
            ...
            continue
        try:
            attachment_document_id = await upload_artifact(
                client,
                settings,
                conn,
                metadata,
    ...
    if parent_document_id:
        await patch_main_complaint_attachments(
            client,
            settings,
            metadata,
            parent_document_id,
    ...
    for snapshot_path in snapshots:
        ...
        for role in roles_for_preflight(field_config):
            ...
            for field_name, raw_value in resolved_role_field_config(
                ...
                data_type = field.get("data_type")
                if (
                    field_name in {"Complaint Attachments", "Parent Case"}
                    and data_type != "documentlink"
                ):
                    issues.add(
                        f"Paperless custom field {field_name} must be documentlink, got {data_type}"
                    )
                    continue
                value = resolve_config_value(raw_value, context)
                value = apply_field_mapping(field_config, field_name, value)
...
async def run_paperless_check_async(
    ...
    try:
        snapshots = snapshots_for_paperless_scope(settings, conn)
    finally:
        conn.close()
